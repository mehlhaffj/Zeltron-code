...
def plot_currents_slice(*args, **kwargs):
    it = args[0]
    ...
    Jze = pu.readArrayFromHdf5(os.path.join(currentdir, "Jz_electrons_%s.h5" % it), "field")
    Jye = pu.readArrayFromHdf5(os.path.join(currentdir, "Jy_electrons_%s.h5" % it), "field")
    Jxe = pu.readArrayFromHdf5(os.path.join(currentdir, "Jx_electrons_%s.h5" % it), "field")
    Jzi = pu.readArrayFromHdf5(os.path.join(currentdir, "Jz_ions_%s.h5" % it), "field")
    Jyi = pu.readArrayFromHdf5(os.path.join(currentdir, "Jy_ions_%s.h5" % it), "field")
    Jxi = pu.readArrayFromHdf5(os.path.join(currentdir, "Jx_ions_%s.h5" % it), "field")

    Jz = Jzi + Jze
    Jy = Jyi + Jye
    Jx = Jxi + Jxe

    Jz = Jz[dslice].squeeze()
    Jy = Jy[dslice].squeeze()
    Jx = Jx[dslice].squeeze()
    ...
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, Jx/fieldnorm,
        cmap = cmap,
        shading = "auto",
        vmin = cmin, vmax = cmax
    )
    ...
    ax.text(
        texth, textv, r"$J_x/%s$" % fieldnormstr,
        ha = textha, va = textva,
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, Jy/fieldnorm,
        cmap = cmap,
        shading = "auto",
        vmin = cmin, vmax = cmax
    )
    ...
    ax.set_ylim((vmin_plot, vmax_plot))
    ax.text(
        texth, textv, r"$J_y/%s$" % fieldnormstr,
        ha = textha, va = textva,
        path_effects = path_effects,
    )
    ax.set_aspect("equal")
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, Jz/fieldnorm,
        cmap = cmap,
        shading = "auto",
        vmin = cmin, vmax = cmax
    )
    ...
    ax.set_ylim((vmin_plot, vmax_plot))
    ax.text(
        texth, textv, r"$J_z/%s$" % fieldnormstr,
        ha = textha, va = textva,
        path_effects = path_effects,
    )
    ax.set_aspect("equal")
    ...
    #===========================================================================
    fname=".././data/plots/currents_%s_%s.png" % (slicestr, it)
...
if len(args) == 0:
    print("usage: python plot_currents_slice.py timestep [keywords]")
    ...
    # print("  smooth = 0 (no smoothing), 1, 2, 3, ...")
    sys.exit(0)
plot_currents_slice(*args,**kwargs)
