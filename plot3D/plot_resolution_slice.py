...
def plot_resolution_slice(*args, **kwargs):
    it = args[0]
    ...
    Ubge = pu.readArrayFromHdf5(os.path.join(densitydir, "Ue_electrons_bg_%s.h5" % it), "field")
    Ubgi = pu.readArrayFromHdf5(os.path.join(densitydir, "Ue_ions_bg_%s.h5" % it), "field")
    Uje  = pu.readArrayFromHdf5(os.path.join(densitydir, "Ue_electrons_drift_%s.h5" % it), "field")
    Uji  = pu.readArrayFromHdf5(os.path.join(densitydir, "Ue_ions_drift_%s.h5" % it), "field")
    ...
    Uj  = Uj[dslice].squeeze()
    Ubg = Ubg[dslice].squeeze()

    if need_transpose:
        nj  = np.transpose(nj)
        nbg = np.transpose(nbg)
        Uj  = np.transpose(Uj)
        Ubg = np.transpose(Ubg)
        Bz = np.transpose(Bz)
        By = np.transpose(By)
        Bx = np.transpose(Bx)
    ...
    rho0 = np.zeros(bsqr.shape)
    ixnz = np.where(bmag > 0.0)
    rho0[ixnz] = me*c*c / (e*bmag[ixnz])

    sigma = np.zeros(bsqr.shape)
    ixnz = np.where(ntot > 0.0)
    sigma[ixnz] = bsqr[ixnz] / (4 * np.pi * me * c * c * ntot[ixnz])

    avggam = np.zeros(ntot.shape)
    ixnz = np.where(ntot > 0.0)
    avggam[ixnz] = Utot[ixnz] / (me * c * c * ntot[ixnz])

    dnonrel = rho0 * np.sqrt(sigma)
    drel    = np.sqrt(avggam) * dnonrel

    ngjlocal = ngj * bmag / B0
    kappa = np.zeros(ntot.shape)
    ixnz = np.where(ntot > 0.0)
    kappa[ixnz] = ntot[ixnz] / ngjlocal[ixnz]
    ...
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, rho0/dx,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, dnonrel/dx,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    ax.set_ylim((vmin_plot, vmax_plot))
    ax.text(
        texth, textv, r"$d_{\rm nonrel}/\Delta x$",
        ha = textha, va = textva,
        path_effects = path_effects,
    )
    ax.set_aspect("equal")
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, avggam * rho0 / dx,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    ax.set_ylim((vmin_plot, vmax_plot))
    ax.text(
        texth, textv, r"$\langle \gamma \rangle \rho_0 / \Delta x$",
        ha = textha, va = textva,
        path_effects = path_effects,
    )
    ax.set_aspect("equal")
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, kappa,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, drel / dx,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    ax.set_ylim((vmin_plot, vmax_plot))
    ax.text(
        texth, textv, r"$d_{\rm rel} / \Delta x$",
        ha = textha, va = textva,
        path_effects = path_effects,
    )
    ax.set_aspect("equal")
    ...
    ax.grid(False)
    pm = ax.pcolormesh(
        hcoords/hnorm, vcoords/vnorm, sigma,
        cmap = cmap,
        shading = "auto",
        norm = norm,
        # vmin = cmin, vmax = cmax
    ...
    #===========================================================================
    fname=".././data/plots/resolution_%s_%s.png" % (slicestr, it)
...
if len(args) == 0:
    print("usage: python plot_resolution_slice.py timestep [keywords]")
    ...
    # print("  smooth = 0 (no smoothing), 1, 2, 3, ...")
    sys.exit(0)
plot_resolution_slice(*args,**kwargs)
