...
def avg_gamma(theta):
    # return theta + 0.5*(kn(3,1.0/theta) + kn(1,1.0/theta))/kn(2,1.0/theta)
    return 3*theta + kn(1,1.0/theta)/kn(2,1.0/theta)


def print_quantities(qsd, full_precision = False):
    max_symbol_len = 0
    max_desc_len = 0
    for tup in qsd:
...
if __name__ == "__main__":
    ...
    # center
    sigbg = 2.0**6  # 2.0**4  # 3.0**4
    # Temperature of the initially stationary background plasma
    thbg  = 2.0 / np.sqrt(sigbg)  # 1.0 / np.sqrt(sigbg)
    # Average Lorentz factor associated with bg plasma
    avggambg = avg_gamma(thbg)
    ...
    rc = rmp / rlc * np.sqrt(sigbg / avggambg) * (1. / (1. + rmp**2 / rnozzle**2)) * rmp

    # Cold magnetization of injected/jet plasma along the last nozzle-attached
    # field line
    muinj = (rmp / rlc)**2 * rnozzle**2 / (rmp**2 + rnozzle**2) * siginj 

    # The critical radius along the last nozzle-attached field line where the
    # flow, according to Michel's solution, goes supermagnetosonic
    rms = rlc * (muinj)**(1./3)

    # Boundaries of the simulation domain
    xmax = 2*Nx  # 4 * rc  # 4*Nx  # 4 * rc
    xmin = -xmax
    Lx = xmax - xmin
    ...
    quantities_symbols_descriptions = [
        ...
        (avggambg, "<gamma>_bg", "Background average Lorentz factor"),
        ...
        (rc / rlc, "R_c / R_LC", ""),
        (muinj, "mu_inj", "Michel parameter of jet plasma"),
        (rms, "R_MS", "Magnetosonic radius"),
        (rms / rlc, "R_MS / R_LC", ""),
        (rc / rms, "R_c / R_MS", ""),
        (4 * rc / rlc, "4 * R_c / R_LC", ""),
